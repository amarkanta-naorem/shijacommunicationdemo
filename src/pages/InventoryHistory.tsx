...
import { Search, ArrowDownToLine, ArrowUpFromLine, MoreHorizontal } from 'lucide-react';
import Header from '../components/Header';
import Badge from '../components/Badge';
import { inventoryHistory } from '../data/mockData';

type TxnType = 'all' | 'in' | 'out';
          ...
          <div className="bg-white rounded-xl border" style={{ borderColor: '#E2E8F0' }}>
            ...
            <div className="flex flex-wrap items-center gap-3 px-4 py-3 border-b" style={{ borderColor: '#F1F5F9' }}>
              ...
              <div className="flex items-center gap-1">
                {([['all', 'All'], ['in', 'In'], ['out', 'Out']] as [TxnType, string][]).map(([v, label]) => (
                  ...
                  {filtered.map(t => (
                    <tr key={t.id + t.type} className="border-t hover:bg-blue-50/30" style={{ borderColor: '#F1F5F9' }}>
                      ...
                      <td className="px-3 py-3">
                        <Badge label={t.type === 'in' ? 'Stock In' : 'Stock Out'} variant={t.type === 'in' ? 'success' : 'danger'} />
                      </td>
